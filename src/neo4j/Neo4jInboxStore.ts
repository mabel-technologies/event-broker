...
export async function ensureProcessedEventConstraint(driver: Driver, database?: string): Promise<void> {
...
export async function purgeExpiredProcessedEvents(driver: Driver, database?: string): Promise<number> {
  const session = driver.session({ database });
  try {
 ...
 */
export class Neo4jInboxStore implements IdempotencyStore {
  constructor(
  ...
  async claim(key: string, token: string, leaseMs: number): Promise<ClaimResult> {
    ...
    try {
      const r = await session.executeWrite((tx) =>
        tx.run(PROCESSED_EVENT_CLAIM_CYPHER, { key, token, now, leaseUntil: now + leaseMs, abandonedTtl: now + ABANDONED_ROW_TTL_MS }),
      );
      const rec = r.records[0];
      const state = rec?.get("state");
      const lockToken = rec?.get("lockToken");
      const expiresAt = Number(rec?.get("expiresAt") ?? 0);
      if (state === "PROCESSING" && lockToken === token) return "claimed";
      if (state === "PROCESSED" && expiresAt > now) return "done";
      return "in_flight";
    } finally {
      await session.close();
    }
  ...
  async complete(key: string, token: string, retentionSeconds: number): Promise<void> {
    ...
    try {
      await session.executeWrite((tx) =>
        tx.run(PROCESSED_EVENT_COMPLETE_CYPHER, { key, token, now: Date.now(), expiresAt: Date.now() + retentionSeconds * 1000 }),
      );
    } finally {
      await session.close();
    }
