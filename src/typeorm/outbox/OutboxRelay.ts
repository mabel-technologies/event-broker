...
import { newLockToken, type OutboxRepo } from "./OutboxRepo.js";
...
export class OutboxRelay {
  constructor(
    ...
    private readonly publish: (eventName: string, payload: { eventId: string } & Record<string, unknown>) => Promise<void>,
  ...
  async runOnce(): Promise<OutboxRelayResult> {
    ...
    const rows = await this.repo.claimBatch({
      ...
      lockToken,
      leaseMs: this.opts.leaseMs ?? 30_000,
    ...
    for (const row of rows) {
      try {
        await this.publish(row.eventName, row.payload as { eventId: string } & Record<string, unknown>);
