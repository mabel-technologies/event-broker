...
export function OnSubscribe(eventName: string, opts: OnSubscribeOptions = {}) {
  return function (target: object, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (this: object, payload: unknown, ...args: unknown[]) {
      ...
      const retention = config?.idempotency?.retentionSeconds ?? 14 * 86_400;
      ...
      try {
        ...
        throw new RetryableEventError(`${LOG_PREFIX} idempotency store unavailable: ${(err as Error)?.message ?? String(err)}`, err);
      }
